...
def simulate(particles, dt, time_limit, skips, ep_squared):
...
def calculate_energies(masses, time_array, positions, velocities):
...
def output_energies(time, E_k, E_p , E_tot):
    ...
    print "U/K start:", initial_p/initial_k
...
if __name__ == "__main__":
    ...
    #time_limit = int(10000*timestep)
    timestep = 24*60*60
    time_limit = int(365*timestep)
    skips = 1
    frames = int(time_limit/(timestep*skips))
    masses = particles[:,0]
    n = len(masses)
    a = 0
    epsilon = a*0.98*(n)**(-0.26)
    print "epsilon:", epsilon
    ep_squared = epsilon**2
    time, positions, velocities = simulate(particles, timestep, time_limit, skips, ep_squared)
    #output1(simulate(particles, timestep, time_limit, skips))
    output2(positions, frames, a)
    k_energies, p_energies, tot_energies = calculate_energies(masses, time, positions, velocities)
    output_energies(time, k_energies, p_energies, tot_energies)
