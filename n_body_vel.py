...
def average_velocity(vel, pos, rad):
    v = []
    for body in range(1, len(pos[:,0])):
        x0 = pos[body, 0]
        y0 = pos[body, 1]
...
if __name__ == "__main__":
    ...
    for i in range(n_sims):
        ...
        v[i] = average_velocity(vel[0], pos[0], 1.3*a)
    print np.mean(v)
    print np.std(v)/np.sqrt(n_sims)
