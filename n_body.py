...
def calculate_energies(masses, time_array, positions, velocities, a):
    ...
    print "total energy start:", str(tot_energies[0])
    return k_energies, p_energies, tot_energies

def plot_energies(time_long, E_k, E_p , E_tot):
    t = time_long[0:len(time_long)-1]
    time_gyr = t/(1000000000*365*24*60*60)
    virial = E_p + 2 * E_k
    plt.figure()
    plt.plot(time_gyr, E_k, label="Kinetic energy")
    plt.plot(time_gyr, E_p, label="Potential energy")
    plt.plot(time_gyr, E_tot, label="Total energy")
    plt.plot(time_gyr, virial, label="U + 2K")
    plt.xlabel("Time/Gyr")
    plt.xlim(0, time_gyr[-1])
    plt.ylabel("Energy/J")
    #plt.legend(loc='best')

    initial_energy = E_tot[0]
    final_energy = E_tot[len(E_tot)-1]
    percent_change = abs(100*(final_energy-initial_energy)/initial_energy)
    print "change in total energy is", str(percent_change) + "%"
    initial_k = E_k[0]
    final_k = E_k[len(E_tot)-1]
    initial_p = E_p[0]
    final_p = E_p[len(E_tot)-1]
...
def plot_distributions(positions, M, a, masses, step_nums):
    densities = []
    for i in range(len(step_nums)):
        section_mids, density = calculate_distribution(centre(positions[step_nums[i],:,:], masses), M, a, masses)
    ...
    for i in range(len(step_nums)):
        ax.plot(section_mids[2:-1]/a, densities[i][2:-1])
        ax.scatter(section_mids[2:-1]/a, densities[i][2:-1])
    ax.plot(x/a, y)
    ax.set_xscale('log')
    ax.set_xlim([x_start/a,x_end/a])
    plt.show()
...
def calculate_yonks(positions, t, a):
    yonked_bodies = np.zeros(len(positions[0,:,0]))
    yonks = np.zeros(len(t))
    for t in xrange(len(t)):
        for body in xrange(len(positions[0,:,0])):
            x = positions[t,body,0]
            y = positions[t,body,1]
            z = positions[t,body,2]
            r_squared = x**2 + y**2 + z**2
            if r_squared > (a*5)**2:
                yonked_bodies[body] = 1
        yonks[t] = np.sum(yonked_bodies)
    return yonks

def plot_yonks(positions, t, a):
    yonks = calculate_yonks(positions, t, a)
    plt.figure()
    plt.plot(t, yonks-yonks[0])
    plt.show()
...
if __name__ == "__main__":
    ...
    t, positions, velocities = simulate(particles, timestep, time_limit, skips, ep_squared, M)
    #output1(simulate(particles, timestep, time_limit, skips))
    #output2(positions, frames, a)
    k_energies, p_energies, tot_energies = calculate_energies(masses, t, positions, velocities, a)
    plot_energies(t, k_energies, p_energies, tot_energies)
    plot_distributions(positions, M, a, masses, [0, (steps/skips)-1])
    plot_yonks(positions, t, a)
