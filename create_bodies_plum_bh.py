...
if __name__ == "__main__":
    N = 500
    m_sun = 2e30
    star_mass = (float(175)/float((249+125)))*m_sun
    print float(175)/float((249+125))
    print float(175)/(float((249+125))*2)
    masses = np.zeros(N)
    for i in range(N/2):
        masses[i] = star_mass
    for i in range(N/2, N):
        masses[i] = star_mass/2
    masses[0] = 200*m_sun
    a = 2*pc          # plummer radius
    M = np.sum(masses)
    print M
    #create_file("initial_conditions_bh.txt", N, masses, a, M)
